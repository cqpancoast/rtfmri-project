...
# DMN and CEN nodes
#dmn_nodes = [48, 55] #(reduced)
#cen_nodes = [6, 7] #(reduced)
dmn_nodes = [48, 55, 56, 79, 80, 26, 27, 28, 29, 30, 31, 91, 92, 114, 115, 116, 117, 118, 119, 120, 121]
cen_nodes = [0, 1, 4, 5, 6, 7, 34, 35]
...
# Generates the degree distribution of the minimum spanning trees generated by
# the density value if it is provided, genereates the strength distribution of
# the original graphs otherwise.
...
def generate_conn_vs_trial(G_split, region):
    
    G_split_processed = process_diagnoses(G_split, lambda G: regional_connectivity(G, region))
    ...
    def extract_runs(l):
        ...
        #6: restpost
        #7: restpre
        ordered_averages = [l[3], l[5], l[4], l[2]]
...
def bin_deg_dist(P_k):
    binned_dd = {'NT': {}, 'SZ': {}}
    
    binned_dd['NT'] = log_bin_hist(P_k['NT'])
...
def plot_dicts(dicts_):
    ...
    #Dict of lists
    elif isinstance(dicts_, dict) and isinstance(list(dicts_.values())[0], list):
        for list_ in dicts_.values():
            ax.plot(range(1, len(list_)+1), list_)
    #Dict of dicts
